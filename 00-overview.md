...
**Generazione parallela del piano.** Due modelli linguistici distinti, selezionati per minimizzare la correlazione degli errori, generano indipendentemente un piano d'azione a partire dallo stesso intent strutturato. La generazione è parallela per motivi di latenza.

**Pipeline di validazione per canale.** Ogni piano generato attraversa una pipeline di validazione che procede per stadi: una pre-validazione sintattica rapida che scarta output incomprensibili, un sanitizer che corregge piccole deviazioni recuperabili, una validazione sintattica completa tramite JSON Schema, una validazione semantica che verifica la coerenza del piano con l'intent e le regole di dominio, e un ottimizzatore che porta il piano in forma canonica.

**Binding logico.** Prima del confronto, ogni nodo del piano viene arricchito con informazioni sul tipo di implementazione da usare, una decisione semantica basata sull'intent che può essere presa deterministicamente e che deve concordare tra i due canali.

**Confronto tra i canali.** I due piani canonici vengono confrontati. Se concordano, si procede. Se divergono, il sistema categorizza il disaccordo e decide il comportamento conseguente, che va dal retry all'escalation verso un operatore umano.
